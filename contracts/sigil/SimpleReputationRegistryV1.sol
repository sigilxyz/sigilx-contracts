...
contract SimpleReputationRegistryV1 is UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    struct Feedback { address reviewer; int128 value; uint8 valueDecimals; string tag1; string tag2; string feedbackURI; bytes32 feedbackHash; uint64 timestamp; bool isRevoked; }
    struct Summary { uint64 count; int128 summaryValue; }
    ...
    function giveFeedback(uint256 agentId, int128 value, uint8 valueDecimals, string calldata tag1, string calldata tag2, string calldata, string calldata feedbackURI, bytes32 feedbackHash) external whenNotPaused {
        if (value < -1000 || value > 1000) revert ValueOutOfBounds(value);
        uint64 globalIndex = uint64(_feedback[agentId].length);
        _feedback[agentId].push(Feedback({ reviewer: msg.sender, value: value, valueDecimals: valueDecimals, tag1: tag1, tag2: tag2, feedbackURI: feedbackURI, feedbackHash: feedbackHash, timestamp: uint64(block.timestamp), isRevoked: false }));
    ...
    function getSummary(uint256 agentId) external view returns (uint64 count, int128 summaryValue) { Summary storage s = _summary[agentId]; return (s.count, s.summaryValue); }
    function readFeedback(uint256 agentId, address clientAddress, uint64 feedbackIndex) external view returns (int128 value, string memory tag1, bool isRevoked) {
    ...
    function totalFeedbackCount(uint256 agentId) external view returns (uint256) { return _feedback[agentId].length; }
}
